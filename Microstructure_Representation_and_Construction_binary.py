...
if __name__ == "__main__":
    ...
    for j, alpha in enumerate(alpha_list):
        ...
        ax_top.set_title(r"$\alpha = $" + f"{alpha}", fontsize=12)
        ax_top.set_xlabel(r"$kₓ$")
        ax_top.set_ylabel(r"$kᵧ$")
        ...
        ax_mid.set_xlabel("x")
        ax_mid.set_ylabel("y")
        fig.colorbar(im_mid, ax=ax_mid, fraction=0.046, pad=0.04)
    ...
    out_file = os.path.join(out_folder, "composite_field_cleaned_binary1.png")
    plt.savefig(out_file, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"Saved cleaned figure to: {out_file}")